# Exact NAFNet used in the project.
# Configuration: 1-channel input, width=32, middle_blk_num=2,
...
class NAFBlock(nn.Module):
    ...
    def forward(self, inp):
        x = self.norm1(inp)
        x = self.conv1(x); x = self.conv2(x); x = self.sg(x); x = x*self.sca(x); x = self.conv3(x); x = self.dropout1(x)
        y = inp + x*self.beta
...
if __name__ == '__main__':
    ...
    model=NAFNet(img_channel=1,width=32,middle_blk_num=2,enc_blk_nums=[1,1,1,1],dec_blk_nums=[1,1,1,1]).to(device)
