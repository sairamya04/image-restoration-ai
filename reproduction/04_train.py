...
def gradient_loss(pred, target):
    ...
    pred_x = F.conv2d(
        pred, sobel_x, padding=1
    )

    pred_y = F.conv2d(
        pred, sobel_y, padding=1
    )

    target_x = F.conv2d(
        target, sobel_x, padding=1
    )

    target_y = F.conv2d(
        target, sobel_y, padding=1
    )
...
for epoch in range(NUM_EPOCHS):
    ...
    print(
        f"       "
        f"L1={train_l1:.6f} "
        f"Grad={train_grad:.6f} "
        f"Freq={train_freq:.6f}"
    )
