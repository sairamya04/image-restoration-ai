...
def load_image(path: Path) -> np.ndarray:
    ...
    if array.ndim != 2:
        raise ValueError(
            f"Expected a single-channel image: {path} -> {array.shape}"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate the trained image-restoration model."
    )
    parser.add_argument(
        "--input-dir",
        required=True,
    ...
    parser.add_argument(
        "--gt-dir",
        default=None,
        help="Optional ground-truth directory for PSNR/SSIM.",
    )
    ...
    for path in files:
        ...
        if gt_path is None:
            print(
                f"Warning: no ground truth found for {path.name}"
            )
            continue
        ...
        if gt.shape != restored.shape:
            raise ValueError(
                f"Shape mismatch for {path.stem}: "
                f"restored={restored.shape}, gt={gt.shape}"
            )

        mse = float(
            np.mean((restored - gt) ** 2)
        )
        psnr = 10.0 * np.log10(
            1.0 / (mse + 1e-10)
        )
    ...
    print(
        f"Mean inference   : "
        f"{total_ms / len(files):.3f} ms/image"
    )
